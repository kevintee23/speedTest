...
sudo echo "ExecStart=/usr/bin/gunicorn -b 0.0.0.0:5000 speedtest:app" >> gunicorn-speedtest.service
...
sudo echo "   return subprocess.Popen('/usr/local/bin/fast-speedtest yourFastSpeedTestToken', shell=True, stdout=subprocess.PIPE).stdout.read()" >> speedtest.py
...
sudo cp /home/pi/$stdir/gunicorn.service /etc/systemd/system/
